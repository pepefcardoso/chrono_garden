...
1. `T-001` — Modelagem Imutável (Freezed) — **Garante segurança de não ter estado corrompido.**
2. `T-002` — Criação do Notifier de Time Travel — **A espinha dorsal da mecânica principal.**
3. `T-003` — Motor Determinístico no `tick()` — **Onde a lógica real de jogo vive.**
4. `T-004` — Criação dos níveis (Hardcoded JSON) — **Necessário para alimentar a UI no Sprint 2.**
